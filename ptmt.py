...
df = pd.read_csv('salary_prediction/data/adult1.csv')
...
# Kiểm tra kết quả
print(f"\nSố cột trước encoding: {len(df_clean.columns)}")
print(f"Số cột sau encoding: {len(df_encoded.columns)}")
# Chuyển tất cả các cột kiểu bool về 1/0
bool_cols = df_encoded.select_dtypes(include='bool').columns
df_encoded[bool_cols] = df_encoded[bool_cols].astype(int)

df_encoded.to_csv('salary_prediction/data/adult2.csv', index=False)
print("\nDữ liệu đã được lưu vào 'adult2.csv'")
